...
class _FlashcardScreenState extends State<FlashcardScreen> {
  ...
  Future<void> generateFlashcards({bool isFromPdf = false}) async {
    ...
    setState(() { loading = true; loadingStatus = "AI is building your deck..."; FocusScope.of(context).unfocus(); cards = []; });
    ...
    if (cards.isEmpty) {
      return Column(
        ...
        children: [
          ...
          Text("Your deck is empty", style: TextStyle(color: Colors.grey.shade500, fontSize: 18)),
        ],
      );
    }
    ...
    return CardSwiper(
      ...
      onSwipe: (previousIndex, currentIndex, direction) {
        setState(() {
          cardsRemaining--; 
          if (cardsRemaining == 0) {
             // Reward XP for finishing the deck
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Study Deck", style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                hintText: "Paste notes or upload a PDF to build your deck...",
            ...
            Row(
              children: [
                ...
                Expanded(flex: 2, child: ElevatedButton.icon(onPressed: loading ? null : () => generateFlashcards(), icon: loading ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.auto_awesome), label: Text(loading ? "Working..." : "Generate Cards"), style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14), backgroundColor: const Color(0xFF4F46E5), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))),
              ],
            ),
            ...
            if (!loading && cards.isNotEmpty && cardsRemaining > 0)
              Padding(padding: const EdgeInsets.only(top: 20.0), child: Text("$cardsRemaining cards remaining", style: TextStyle(color: Colors.grey.shade600, fontWeight: FontWeight.w600))),
            if (!loading && cards.isNotEmpty && cardsRemaining == 0)
              const Padding(padding: EdgeInsets.only(top: 20.0), child: Text("Deck Completed! 🎉 +20 XP", style: TextStyle(color: Color(0xFF10B981), fontWeight: FontWeight.bold, fontSize: 16))),
          ],
        ),
      ),
    );
